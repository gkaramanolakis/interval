...
def get_code_fnames():
    ret = []
    src_files = Path('../iws').rglob('*.py')
    for srcf in src_files:
        src_path = os.path.abspath(srcf)
        ret.append(src_path)

    src_files = Path('../iws').rglob('*.sh')
    for srcf in src_files:
        src_path = os.path.abspath(srcf)
        ret.append(src_path)

    src_files = Path('../scripts').rglob('*.sh')
    for srcf in src_files:
        src_path = os.path.abspath(srcf)
        ret.append(src_path)
    return ret
...
def backup_src(args):
    # save source code as a zip file under logdir
    import zipfile

    backup_dir = os.path.join(args.logdir, 'code.zip')
    outzip = zipfile.ZipFile(backup_dir, 'w')
    files = get_code_fnames()
    for file in files:
        savefile = 'src/' + os.path.relpath(file, backup_dir).replace('../', '')
        outzip.write(file, savefile, compress_type=zipfile.ZIP_DEFLATED)
    outzip.close()
    return

def backup_src2(args):
    src_files = Path('../iws').rglob('*.py')
    for srcf in src_files:
        src_path = os.path.abspath(srcf)
        fname = srcf.name
        parent_dir = srcf.parent.absolute()
        srcf_file_dir = str(parent_dir).split('/code/')[-1]
        backup_dir = os.path.join(args.logdir, 'src/{}'.format(srcf_file_dir))
        os.makedirs(backup_dir, exist_ok=True)
        tgt_path = os.path.join(backup_dir, fname)
        # print("{} -> {}".format(src_path, tgt_path))
