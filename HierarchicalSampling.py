...
def run_qs_old(trn_ds, qs, truth, quota):
...
class HierarchicalSampling(QueryStrategy):
    ...
    def __init__(self, dataset, classes, active_selecting=True, subsample_qs=None, random_state=None, logger=None):
        super(HierarchicalSampling, self).__init__(dataset)
        self.logger = logger
        X, _ = self.dataset.get_entries()
        cluster = AgglomerativeClustering()

        self.logger.info(f"Creating hierarchical clustering ({len(X)} data)")
        cluster.fit(X)
    ...
    def update(self, entry_id, label):
        if label not in self.class_id:
            raise ValueError('Unknown class of entry %d: %s, expected: %s' % (entry_id, label, list(self.class_id.keys())))
    ...
    @inherit_docstring_from(QueryStrategy)
    def make_query(self, proba=None, num_samples=1, offset=None):
        """

        :param proba: model predicted probabilities on all (unlabeled + labeled) dataset
        ...
        if self.sub_qs is None:
            ask_ids = [int(self._sample_node(pruning))]
        else:
            _, scores = self.sub_qs.make_query(proba=proba, return_score=True)
            leaves = set(self._find_leaves(pruning))
            leaf_scores = [(score, node) for node, score in scores if node in leaves]
            ask_ids = [max(leaf_scores)[1]]
        return ask_ids
    ...
    def _update(self, entry_id, label, root_pruning):
        ...
        while node != NO_NODE:
            ...
            self.total[node] += 1
            assert self.total[node] <= self.size[node], f"{self.total[node]} > {self.size[node]}"
...
class UncertaintySampling(QueryStrategy):
    ...
    Parameters
    ----------
    [OLD] model: :py:class:`libact.base.interfaces.ContinuousModel` or :py:class:`libact.base.interfaces.ProbabilisticModel` object instance
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.method not in ['lc', 'sm', 'entropy']:
            raise TypeError("supported methods are ['lc', 'sm', 'entropy'], the given one "
                "is: " + self.method
            )

    def _get_scores(self, proba):
        dataset = self.dataset
        unlabeled_entry_ids, _ = dataset.get_unlabeled_entries()
        dvalue=proba[unlabeled_entry_ids]
    ...
    def make_query(self, proba, return_score=False):
        ...
        unlabeled_entry_ids, scores = zip(*self._get_scores(proba))
    ...
    def _get_scores_new_bad(self, proba):
        dvalue = proba
    ...
    def make_query_new_bad(self, proba, return_score=False, num_samples=1):
        """Return the index of the sample to be queried and labeled and
        selection score of each sample. Read-only.
        ...
        scores = self._get_scores(proba)
        ask_id = get_sampled_ind(scores=scores, num_samples=num_samples, reverse=False)
        return_ids = ask_id
        if return_score:
            return return_ids, scores
        else:
            return return_ids


def get_sampled_ind(scores, num_samples, reverse):
    sampled_ind = np.argpartition(scores, -num_samples)[-num_samples:]
    if reverse:
            sampled_ind = np.argpartition(scores, num_samples)[:num_samples]
    return sampled_ind